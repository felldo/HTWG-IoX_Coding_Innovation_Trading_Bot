...
class MACD:
    ...
    def trade(self, open_time, close_price):
        ...
        for index, (_, row) in enumerate(data.iterrows()):
            # Buy if MACD > Singal and intersection
            if row[1] > row[2]:
                # If it isn't bought and isn' an intersection and the previous row is no intersection go on
                if row[4] == 0 and row[5] == 0 and self.prev_row[5] != 2:
                    ...
                    # If its not a intersection point
                    else:
                ...
                # Else we are just between two intersections, so set the Flag to 2
                    ...
                    self.close_price_list_pd.loc[index, 'intersection'] = 2
            # Else Sell if Singal > MACD
            elif row[2] > row[1]:
                # If the current isn't bougth and the prev row has an intersection Flag set to 2
...
if __name__ == '__main__':
    ...
    macd.get_macd(macd.close_price_list_pd)
